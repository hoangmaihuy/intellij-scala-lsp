...
class CallHierarchyProviderIntegrationTest extends ScalaLspTestBase:
  ...
  def testIncomingCallsFromRangesAreNonEmpty(): Unit =
    ...
    assertTrue("Should return at least one prepared item", items.nonEmpty)
    val incoming = provider.incomingCalls(items.head)
    assertTrue("Should have at least one incoming call", incoming.nonEmpty)
    val call = incoming.find(_.getFrom.getName == "callerA")
    assertTrue("Should find callerA", call.isDefined)
    val fromRanges = call.get.getFromRanges
  ...
  def testOutgoingCallsFromRangesAreNonEmpty(): Unit =
    ...
    assertTrue("Should return at least one prepared item", items.nonEmpty)
    val outgoing = provider.outgoingCalls(items.head)
    assertTrue("Should have at least one outgoing call", outgoing.nonEmpty)
  ...
  def testNestedCallsChain(): Unit =
    ...
    assertTrue("Should return at least one prepared item for a()", itemsA.nonEmpty)
    ...
    assertTrue("Should return at least one prepared item for b()", itemsB.nonEmpty)
